...
st.set_page_config(page_title="EMOQ-Emotion Based Food Recommendation System",
                   page_icon=":yum:", layout="wide")
name = ""
...
def state_selection():
    ...
    available_states = ("Bored", "Tired", "Lazy",
                        "Work_mode", 'Try_something_new!',)
...
def food_recommendation():
    ...
    for i in range(3):
        st.subheader(str(i + 1) + "." + str(food_recom_list[i]).capitalize())
...
# --- LOAD ASSETS---
lottie_food_choice = load_lottieurl(
    "https://assets1.lottiefiles.com/packages/lf20_ysas4vcp.json")
lottie_food_train = load_lottieurl(
    "https://assets10.lottiefiles.com/packages/lf20_YnsM0o.json")
lottie_emotion = load_lottieurl(
    "https://assets6.lottiefiles.com/packages/lf20_ewpty5.json")
lottie_bubble = load_lottieurl(
    "https://assets5.lottiefiles.com/temp/lf20_ujod0D.json")
lottie_feedback = load_lottieurl(
    "https://assets6.lottiefiles.com/private_files/lf30_ltuqrtmn.json")
        ...
        with st.container():
            ...
            with st.form(key='info'):
                firstname = st.text_input(
                    "FirstName", placeholder="What can i call you?")
                lastname = st.text_input(
                    "LastName", placeholder="What's your last name?")
                email = st.text_input(
                    "Email", placeholder="What's your email address?")
...
if selected == "Food Recommender":
    ...
    left_column, right_column = st.columns(2)
    with left_column:
        st_lottie(lottie_food_choice, height=150,
                  width=200, key="food choice1")
        st.title("EMOQ :bento:")
        st.write("---")
        with st.container():
            food_recom_list = []
            ...
            emotionalstate = st.text_input(
                " ", placeholder="Write here", key="emotionalstateinput")

            if emotionalstate != "":
                food_recom_list = Food_dataset_analysis.find_my_comfort_food(
                    emotionalstate)
                st.subheader("You're " + dominant_emo)
